...
| Field | Value |
|---|---|
| Asset path | `/Game/_ThirdParty/GASP_example/Blueprints/RetargetedCharacters/BP_Echo.BP_Echo` |
| Asset name | `BP_Echo` |
| Subclass | `Blueprint` |
...
- `DefaultSceneRoot` — `SceneComponent` [root] @ socket `None`
  - `EchoBody` — `SkeletalMeshComponent` @ socket `None`
    - `EchoHair` — `SkeletalMeshComponent` @ socket `None`
